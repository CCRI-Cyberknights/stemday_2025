...
def extract_and_decode(passwords, segments_dir: Path, extracted_dir: Path, decoded_dir: Path):
    ...
    for idx, password in enumerate(passwords, 1):
        ...
        subprocess.run(["unzip", "-P", password, str(zip_file), "-d", str(extracted_dir)],
...
def assemble_flag(decoded_dir: Path, output_file: Path):
    ...
    lines_per_file = [f.read_text().splitlines() for f in decoded_files]
    candidate_flags = []
...
def main():
    ...
    flag, passwords = load_unlock_data(root, challenge_id).values()
